    ...
    return (
        <Link href={feed.link} target="_blank">
            <div className="bg-white rounded-lg p-4 flex flex-col gap-3 border border-gray-200 cursor-pointer">
                ...
                {feed.image && <div><img className="w-full object-cover" src={feed.image} alt="img" /></div>}
                {feed.description && <p className="text-gray-500 text-xs leading-5">{feed.description}</p>}
